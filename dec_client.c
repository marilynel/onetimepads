...
int main(int argc, char *argv[]) {
    ...
    if (msgBuffer == NULL) {
        fprintf(stderr, "dec_client: error creating buffer\n");
        exit(1);
    }
    size_t result = fread(msgBuffer, sizeof(char), sizeOfMsg, messageFile);
    if (result != sizeOfMsg) {
        fprintf(stderr, "dec_client: error reading file\n");
        exit(1);
    }
    ...
    if (connect(socketFD, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) < 0) {
        fprintf(stderr, "dec_client: error connecting to server\n");
        exit(1);
    }

    // Check for valid client/server connection by sending "dec" to server
    ...
    while (len > 0) {
        if (len < 256) {
            ...
            if (sentChars != len) {
                fprintf(stderr, "dec_client: message data is missing\n");
                exit(1);
            }
            break;
        } else {
    ...
    // Get size of decrypted message from server
    memset(buffer, '\0', 256);
    ...
    int sizeOfdec = atoi(buffer);
    // TODO retry n number of times ???   
    char *decBuffer = calloc(sizeOfMsg, sizeof(char));      // Allocate buffer just for storing the rec'd msg
    int decIndex = 0;
    int decLen = sizeOfdec;
    while (decLen > 0) {
        if (decLen < 256) {                                    // If msg is < 256 chars, or that is all that is left
            charsRead = recv(socketFD, &decBuffer[decIndex], decLen, 0);
            if (charsRead != decLen) {
                fprintf(stderr, "dec_client: message data is missing\n");
                exit(1);
            }
            ...
            charsRead = recv(socketFD, &decBuffer[decIndex], 256, 0);
            decIndex += 256;                                // move index up
            decLen = decLen - 256;                                // move size down
            if (charsRead != 256) {
                fprintf(stderr, "dec_client: message data is missing\n");
                exit(1);
            }
